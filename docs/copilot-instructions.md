...
1.  **R の `DESCRIPTION` の `Version` は数字とピリオドのみ**（例:
    `0.0.2`）。`-alpha` などのサフィックスは付けない。

2.  **RとPython、それぞれ個別のtar.gzをアセットとして添付する**。リポジトリ全体の
    Source code は使わない。
...
3.  **vignette のビルド済みHTMLを `r/rtfreporter/inst/doc/`
    に含めてからtar.gzを作成する**。
