...
\`titles\` must equal the number of pages already added via
\[rtf_tables()\] / \[rtf_figures()\].

## Usage

``` r
rtf_titles(doc, titles)
```
...
\`"BLANK_ROW"\` (full-height blank row, equivalent to \`""\`) are
honoured. Pass \`NULL\` for a single element to fall back to the default
of one \`HALF_BLANK_ROW\` row above the content.
