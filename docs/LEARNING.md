...
because — and only because — multiple
...
The R6 package is declared in `Suggests:`, not `Imports:`.
[`rtf_theme()`](https://ichirio.github.io/rtfreporter/reference/rtf_theme.md)
...
want a runtime dependency on R6 simply ignore
[`rtf_theme()`](https://ichirio.github.io/rtfreporter/reference/rtf_theme.md);
users who want shared mutable themes opt in with
`install.packages("R6")`.
...
- All public objects except
  [`rtf_theme()`](https://ichirio.github.io/rtfreporter/reference/rtf_theme.md)
  are plain S3 lists. `inherits(x, "rtf_border")`, field access via
  `x$top` / `x[["top"]]`, and `unclass(x)` all work as expected.
...
See
[`vignette("class-systems", package = "rtfreporter")`](https://ichirio.github.io/rtfreporter/articles/class-systems.md)
for a hands-on walk-through of the S3 vs R6 design choices in this
package.
